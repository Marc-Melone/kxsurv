...
from .validation import candle_timestamp, number, price, timestamp_us
...
CREATE TABLE IF NOT EXISTS trades (
  ...
  created_time_us INTEGER NOT NULL,
...
def _migrate_schema(conn: sqlite3.Connection) -> None:
    ...
    if "created_time_us" not in _columns(conn, "trades"):
        conn.execute("ALTER TABLE trades ADD COLUMN created_time_us INTEGER")
    # Preserve original API timestamps; add an exact, indexed comparison key.
    # Retry an interrupted backfill instead of leaving NULL comparison keys.
    rows = conn.execute("SELECT trade_id, created_time FROM trades"
                        " WHERE created_time_us IS NULL").fetchall()
    converted = [(timestamp_us(ts), tid) for tid, ts in rows]
    conn.executemany("UPDATE trades SET created_time_us = ? WHERE trade_id = ?",
                     converted)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_trades_ticker_time_us"
                 " ON trades(ticker, created_time_us, trade_id)")
...
def upsert_trades(conn, rows: list[dict]) -> int:
    ...
    for r in rows:
        ...
        record = (
            ...
            number(r.get("count_fp"), "count_fp", positive=True),
            price(r.get("yes_price_dollars"), "yes_price_dollars"),
            price(r.get("no_price_dollars"), "no_price_dollars"), side, side,
            r.get("taker_book_side"), 1 if r.get("is_block_trade") else 0,
            timestamp_us(r["created_time"]),
        )
    ...
    try:
        conn.executemany(
            ...
            " is_block_trade, created_time_us) VALUES (?,?,?,?,?,?,?,?,?,?,?)", payload)
    except Exception:
        conn.rollback()
        raise
    conn.commit()
...
def upsert_candles(conn, rows: list[dict]) -> int:
    ...
    for r in rows:
        ...
        payload.append((
            r["ticker"], candle_timestamp(r.get("end_period_ts")),
            number(r.get("volume_fp"), "volume_fp", minimum=0),
            number(r.get("open_interest_fp"), "open_interest_fp", minimum=0),
            price(bid, "yes_bid.close_dollars") if bid is not None else None,
            price(ask, "yes_ask.close_dollars") if ask is not None else None,
        ))
    conn.executemany(
...
def upsert_markets(conn, rows: list[dict]) -> int:
    payload = [(
        ...
        number(r["floor_strike"], "floor_strike") if r.get("floor_strike") is not None else None,
        r.get("strike_type"),
    ) for r in rows]
    conn.executemany(
